...
function YandexMap({ locale }: { locale: Locale }) {
  const ui = publicUi[locale];
  const mapUrl = "https://yandex.uz/map-widget/v1/?ll=69.218040%2C41.279166&mode=poi&poi%5Bpoint%5D=69.217828%2C41.279319&poi%5Buri%5D=ymapsbm1%3A%2F%2Forg%3Foid%3D40707342179&z=19.8";
  ...
  return <section className="contact-map"><iframe src={mapUrl} title={ui.mapTitle} loading="lazy" allowFullScreen /><a href={fullUrl} target="_blank" rel="noreferrer"><Globe2 />{ui.mapOpen}<ArrowRight /></a></section>;
}
...
export function SolutionsReferencePage() {
  ...
  return <main className="home-page"><SiteHeader />
    ...
    <section id="contacts" className="home-section home-contacts"><div className="site-shell home-contact-grid"><div><span className="section-eyebrow">{ui.contactEyebrow}</span><h2>{ui.contactTitle}</h2><p>{ui.contactText}</p><div className="home-contact-links"><a href={CONTACTS.phoneHref}><Phone/>{CONTACTS.phoneDisplay}</a><a href={CONTACTS.emailHref}><Mail/>{CONTACTS.email}</a></div></div><ContactForm locale={locale} /></div></section>
    <ReferenceFooter />
  </main>;
}
